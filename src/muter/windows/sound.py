...
class AudioDeviceState(enum.Enum):
    Active = 0x1
    Disabled = 0x2
    NotPresent = 0x4
    Unplugged = 0x8
...
class STGM(enum.Enum):
    STGM_READ = 0x00000000


class AUDCLNT_SHAREMODE(enum.Enum):
...
class AudioUtilities:
    ...
    def getMicrophones(self) -> IMMDevice:
        ...
        if audio_input_devices is None:
            logger.error("ERROR: No Active Audio Input devices detected!")
            raise RuntimeError("No Audio Input devices")
